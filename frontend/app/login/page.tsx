      ...
      <div className="mx-auto grid max-w-5xl gap-6 px-4 py-10 md:px-6 lg:grid-cols-[1.05fr_0.95fr] lg:py-14">
        <section className="relative overflow-hidden rounded-3xl border border-slate-200 bg-white p-7 shadow-sm sm:p-8 lg:min-h-[580px]">
          ...
          <h1 className="mt-3 text-3xl font-bold text-slate-900 sm:text-4xl">
            Entra y continua donde te quedaste
          </h1>
          <p className="mt-4 max-w-md text-sm leading-relaxed text-slate-600">
            Tu cuenta te permite pagar mas rapido, ver ordenes y guardar tu progreso de compra.
          ...
          <div className="mt-8 space-y-3">
            <div className="flex items-start gap-3 rounded-xl border border-slate-200 bg-slate-50 p-3">
              ...
              <div>
                <p className="text-sm font-semibold text-slate-900">Sesion protegida</p>
                <p className="text-xs text-slate-600">Cookies seguras y autenticacion centralizada.</p>
              </div>
            </div>
            ...
            <div className="flex items-start gap-3 rounded-xl border border-slate-200 bg-slate-50 p-3">
              ...
              <div>
                <p className="text-sm font-semibold text-slate-900">Checkout privado</p>
                <p className="text-xs text-slate-600">Solo usuarios autenticados pueden pagar y ver ordenes.</p>
              </div>
            </div>
          ...
          {nextPath ? (
            <p className="mt-6 rounded-lg border border-sky-200 bg-sky-50 px-3 py-2 text-xs text-sky-800">
              Despues de iniciar sesion volveras automaticamente a la pagina protegida.
            </p>
          ) : null}
        </section>
        ...
        <section className="flex flex-col rounded-3xl border border-slate-200 bg-white p-7 shadow-sm sm:p-8 lg:min-h-[580px]">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">Acceso rapido</p>
            <h2 className="mt-2 text-2xl font-bold text-slate-900">Iniciar sesion</h2>
          ...
          {!isApiConfigured ? (
            <div className="mt-4 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-800">
              NEXT_PUBLIC_API_BASE_URL no esta configurada. Define esa variable para habilitar el login remoto.
            </div>
          ) : null}

          <div className="mt-auto pt-8 text-xs leading-relaxed text-slate-500">
            Al continuar aceptas nuestras politicas de privacidad y terminos de servicio.
          </div>
        </section>
      </div>
    </div>
