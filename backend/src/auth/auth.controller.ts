...
export class AuthController {
    ...
    logout(@Res() res: Response) {
        ...
        const cognitoLogoutUrl =
            `${domain}/logout?client_id=${clientId}&logout_uri=${logoutRedirect}`;
    ...
    async callback(@Query('code') code: string, @Res() res: Response) {
        ...
        if (!tokenRes.ok) {
            ...
            return res.redirect(
                `${process.env.APP_URL}/?auth=token_failed&details=${encodeURIComponent(err)}`,
            );
        }
    ...
    async me(@Req() req: any) {
        const localUser = await this.usersService.findByCognitoSub(req.user.sub);
